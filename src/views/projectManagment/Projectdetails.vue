...
const projectId = computed(() => route.params.id || props.id);
...
function normalizeProject(data, tasks = []) {
  ...
  return {
    ...
    boardColumns: [
      {
        id: "todo",
        cards: normalizedTasks.filter(t => t.status === "todo"),
      },
      {
        id: "inprogress",
        cards: normalizedTasks.filter(t => t.status === "inprogress"),
      },
      {
        id: "done",
        cards: normalizedTasks.filter(t => t.status === "done"),
      },
    ],
    teamMembers: (data.members || []).map(m => ({
    id: m._id,
    name: m.user?.name || "Unknown",
    avatar: m.user?.avatar || `https://i.pravatar.cc/80?img=${Math.random() * 70}`,
    role: m.role || "Member",
...
function toggleTaskComplete(taskId) {
  project.value.boardColumns.forEach(col => {
...
const isAllDone = computed(() => {
  if (!project.value?.boardColumns) return false;

  const allTasks = project.value.boardColumns.flatMap(c => c.cards);
...
const filteredFiles = computed(() => {
  ...
  let items = Array.isArray(project.value.files)
  ? project.value.files
  : [];
...
<div v-else-if="activeTab === 'Tasks'">
  ...
  <div v-if="teamCount < 4" class="text-center py-20">
    <h2 class="text-xl font-bold text-text-black mb-2">
      Team not ready
    </h2>
    <p class="text-text-gray">
      You need at least 4 team members to access tasks.
    </p>
  </div>
